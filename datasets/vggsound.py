...
class VGGSound(torch.utils.data.Dataset):
    ...
    def __init__(self, split, specs_dir, transforms=None, splits_path='./data', meta_path='./data/vggsound.csv'):
        ...
        #finding directory for codes:
        codes_path = specs_dir.split("/")
        codes_path=codes_path[:-1]
        codes_path[-1] = "codes_10s"
        self.codes_dir_path  = '/'.join(codes_path) ### ./data/vggsound/codes_10s
    ...
    def __getitem__(self, idx):
        ...
        # finding codes for the file
        vid = spec_path.split('/')[-1]
        new_filename = vid.replace('_mel.npy', self.feat_codes_suffix)
        codes_path = os.path.join(self.codes_dir_path, f'{new_filename}')
...
class VGGSoundSpecs(VGGSound):  

    """
    Initializes a VGGSound dataset and converts data into 0 to 1 range.
...
if __name__ == '__main__':
    from transforms import Crop, StandardNormalizeAudio, ToTensor
    specs_path = 'data/vggsound/melspec_10s_22050hz/'
