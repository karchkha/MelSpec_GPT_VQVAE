...
class GaussianLSTMEncoder(GaussianEncoderBase):
    ...
    def forward(self, input):
        ...
        # h_init = torch.ones(1, input.shape[0], self.nh).to(self.args.device) 
        # c_init = torch.ones(1, input.shape[0], self.nh).to(self.args.device) 
        

        _, (last_state, last_cell) = self.lstm(word_embed) #, (h_init, c_init))   ######################### change it here !!!!
